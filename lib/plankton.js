...
function getDestinationFilePath(destDir, post) {
    var urlSlug = "";
    var fileNameWithoutExt = post[constants.FILENAME_PROPERTY].replace(path.extname(post[constants.FILENAME_PROPERTY]),'');
...
Plankton.prototype.publishPost = function(posts, post, callback) {
    ...
    if(postCopy.hasOwnProperty('layout')) {
        ...
        var destPath = getDestinationFilePath(this.destDir, postCopy);
